function x = IRKTemplate(ButcherArray, f, dfdx, T, x0)
    ...
    Nt = length(T);
    x = zeros(Nx, Nt);
    Ns = size(ButcherArray.A, 1);
    %
    %
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    xt = x0; % initial iteration
    k = ones(Ns*Nx,1); % initial guess
    dt = T(2)-T(1);
    x(:,1) = xt;
    ...
    for nt=2:Nt
        ...
        residualFun = @(k) IRKODEResidual(k,xt,T(nt),dt,ButcherArray.A,ButcherArray.c,f);

        jacobianResidual = @(k) IRKODEJacobianResidual(k,xt,T(nt),dt,ButcherArray.A,ButcherArray.c,dfdx);
        k = NewtonsMethod(residualFun, jacobianResidual, k);
        
        %k = NewtonsMethod(residualFun, jacobianResidual, k);
        % Calculate and save next iteration value x_t
        xt = xt + dt* ButcherArray.b.'*k;
        x(:,nt) = xt;
